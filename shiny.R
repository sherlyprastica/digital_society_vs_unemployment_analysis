...
pengangguran <- read.csv("https://raw.githubusercontent.com/sherlyprastica/digital_society_vs_unemployment_analysis/main/fix_pengangguran.csv")
names(pengangguran) <- make.names(names(pengangguran))
pengangguran <- pengangguran %>% filter(Jumlah.Pengangguran > 0)
mean_pengangguran <- pengangguran %>% select(`ï..Tahun`, Jumlah.Pengangguran) %>% mutate(rata_pengangguran = mean(Jumlah.Pengangguran))


user_internet_umur <- read.csv('https://raw.githubusercontent.com/sherlyprastica/digital_society_vs_unemployment_analysis/main/Copy%20of%20pengguna%20internet%20berdasarkan%20umur.csv')
user_internet_umur <- user_internet_umur %>% select(`ï..Umur`, mean)
...
pengangguran_umur <- read.csv("https://raw.githubusercontent.com/sherlyprastica/digital_society_vs_unemployment_analysis/main/pengangguran_umur.csv")
pengangguran_umur <- pengangguran_umur %>% arrange(`ï..umur`)
...
server <- function(input,output){
  observe({
    if(input$feature == "Pengguna Internet 10 Tahun Terakhir"){
      output$plot1 <- renderPlotly({
        viz <- ggplot(data = sepuluh_tahun_trakhir, aes(`ï..Tahun`)) + scale_x_continuous(breaks = c(2011:2020)) + geom_col(aes(y=Jumlah_Penduduk), color="black", fill="lightblue") 
    ...
    if(input$feature == "Rata-rata Umur Pengguna Internet"){
      output$plot2 <- renderPlot({
        viz3 <- ggplot(data=user_internet_umur, aes(x="", y=mean, fill=`ï..Umur`)) 
  ...
  if(input$feature2 == "Kondisi Pengangguran di Indonesia"){
    output$plot3 <- renderPlotly({
      viz4 <- ggplot(data=pengangguran, aes(x=`ï..Tahun`, y=Jumlah.Pengangguran)) + geom_line() + geom_line(data=mean_pengangguran, aes(y=rata_pengangguran), color="red")
  ...
  if(input$feature2 == "Pengangguran di Indonesia Berdasarkan Umur"){
    output$plot3 <- renderPlotly({
      viz6 <- ggplot(data = pengangguran_umur, aes(`ï..umur`)) + geom_col(aes(y=tahun_2020), color="black", fill="lightblue") 
